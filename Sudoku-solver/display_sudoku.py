...
def answer_display(answer: list[list[int]], initial_sudoku: list[list[str]]) -> None:
    mainwindow = tkinter.Tk()
    mainwindow.title('Sudoku')
    mainwindow.geometry('640x480')
    label = tkinter.Label(mainwindow, text="Solution to your Sudoku", font=('Arial', 14, 'bold'))
    label.pack(pady=10)
    ...
    for row in range(9):
        ...
        for col in range(9):
            ...
            if initial_sudoku[row][col].isnumeric():
                entry_sudoku[row][col].config(fg='black')
            entry_sudoku[row][col].insert(0, answer[row][col])
            if col % 3 == 0:
                row_x += 1
    ...
    button1 = tkinter.Button(mainwindow, text='QUIT!', width=50, fg='white', background='black',
